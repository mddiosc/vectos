...
// IsValidMatryoshkaDimension returns true if dim is one of the supported
// Matryoshka truncation sizes.
func IsValidMatryoshkaDimension(dim int) bool {
	for _, d := range MatryoshkaDimensions {
		if d == dim {
			return true
		}
	}
